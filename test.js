...
describe("CuyCollectionNft Testing", async function () {
  ...
  before(async () => {
    ...
    const hashes = [owner, alice, bob, carl, deysi, estefan]
      .map((value, index) => (hashToken(1000 + index, value.address)))
  ...
  it('buyBack should revert for id lower than 1000', async () => {
    ...
    ).to.revertedWith('Buyback no es posible')
  })
  ...
  it('TokenURI not blank', async () => {
    ...
    expect(
      ...
      `ipfs://QmfDR3uXdQDgqH6bxUSfxHgCXeLSTHNGiU1EmhbbEhRLEc/${tokenId}`
    )
  })
...
describe("PublicSale testing", async () => {
  ...
  it('Purchase with tokens, token Id 550', async () => {
    ...
    .withArgs(owner.address, tokenId)
    const calculatedPrice = 10000 + 2000 * Math.floor((new Date().getTime() / 1000 - startDate) / (24*60*60));
    const price = Math.min(90000, calculatedPrice)
  ...
  it('Purchase with USDC', async () => {
    ...
    await expect(
      publicSaleContract.purchaseWithUSDC(tokenId, pEth((60000).toString()))
    ).to.emit(publicSaleContract, 'PurchaseNftWithId')
    .withArgs(owner.address, tokenId)

    expect(await usdContract.balanceOf(owner.address)).to.lessThan(pEth('60000'))
  })
